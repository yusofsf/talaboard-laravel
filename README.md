...
## ایمپورت داده از نسخه‌ی قبلی Flask (shop.db)

دستور `import:flask-shop` کاربران، معاملات، کیف پول، اعلانات و کدهای دعوت را
از `shop.db` نسخه‌ی Flask می‌خواند و در دیتابیس Laravel درج می‌کند (با همان IDها،
برای حفظ ارتباطات بین جدول‌ها):

```bash
php artisan import:flask-shop /path/to/shop.db
# یا اگر FLASK_SHOP_DB_PATH در .env تنظیم شده:
php artisan import:flask-shop
```

دستور idempotent است (با `upsert` روی `id`) — اجرای دوباره خطر تکرار داده ندارد.

### ⚠️ نکته‌ی مهم درباره‌ی رمزهای عبور

رمزهای عبور قدیمی با تابع `werkzeug.security.generate_password_hash` و الگوریتم
**scrypt** هش شده‌اند. PHP به‌صورت بومی هیچ تابع scrypt سازگار با پارامترهای
Python ندارد، پس این هش‌ها **در Laravel قابل تأیید نیستند**.

راه‌حل: کاربران ایمپورت‌شده با `must_reset_password=true` علامت می‌خورند. در
تلاش بعدی ورود، به‌صورت خودکار به صفحه‌ی «بازنشانی رمز عبور» (با OTP) هدایت
می‌شوند؛ پس از تعیین رمز جدید، این پرچم خاموش و `legacy_password_hash` حذف
می‌شود. داده‌های دیگر (نام، موبایل، معاملات، کیف پول، …) بدون تغییر منتقل شده‌اند.

جدول‌های `gold_prices.db` و `silver_prices.db` ایمپورت نمی‌شوند — این‌ها فقط
کش تاریخچه‌ی قیمت بودند و در Laravel با منطق مستقل (`PriceService`) بازسازی
شده‌اند. قیمت نقره هم اکنون مستقیماً و زنده از دیتابیس `sachmebot_laravel`
خوانده می‌شود.
